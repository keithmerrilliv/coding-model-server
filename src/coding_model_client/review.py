...
The two Coding Model judges share the local server's sequential inference
...
from coding_model_client.http import post_chat_completion
...
def _call_local_model(diff: str, agent: str) -> str:
    """Call a local-server Coding Model agent. Used for both reviewer and deep_reviewer."""
...
def fan_out_review(diff: str) -> list[JudgeResult]:
    ...
    Coding Model judges share a single worker thread because the inference lock
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        ...
        for fut in futures:
            ...
            try:
                ...
                if tag == "qwen_pair":
                    msg = "(timed out waiting for both Coding Model judges)"
                    results["reviewer"] = JudgeResult(
                        f"Coding Model reviewer ({CODING_MODEL_REVIEWER_AGENT})", COLORS['GREEN'],
                        msg, REVIEW_TIMEOUT * 2, False,
                    )
                    results["deep_reviewer"] = JudgeResult(
                        f"Coding Model deep_reviewer ({CODING_MODEL_DEEP_REVIEWER_AGENT})", COLORS['HEADER'],
                        msg, REVIEW_TIMEOUT * 2, False,
                    )
                else:
...
def _wrap_qwen_pair(diff: str) -> tuple[JudgeResult, JudgeResult]:
    ...
    name_rev = f"Coding Model reviewer ({CODING_MODEL_REVIEWER_AGENT})"
    name_deep = f"Coding Model deep_reviewer ({CODING_MODEL_DEEP_REVIEWER_AGENT})"

    t0 = time.monotonic()
    try:
        rev_body = _call_local_model(diff, CODING_MODEL_REVIEWER_AGENT)
    ...
    t0 = time.monotonic()
    try:
        deep_body = _call_local_model(diff, CODING_MODEL_DEEP_REVIEWER_AGENT)
