"""coding-model-autonomous CLI — submit specs and respond to review gates.

Thin client over the /v1/autonomous/* endpoints. The coding-model-server hosts the
...
# ── Server config (mirrors coding_model_client/config.py conventions) ────────────────

DEFAULT_HOST = os.getenv("CODING_MODEL_SERVER_IP", "127.0.0.1")
DEFAULT_PORT = int(os.getenv("CODING_MODEL_SERVER_PORT", "5000"))
...
# ── ANSI helpers (subset of coding_model_client.config.COLORS) ───────────────────────
...
def cmd_submit(args: argparse.Namespace) -> int:
    ...
    print(f"  coding-model-autonomous status {resp['spec_id']}")
    print(f"  coding-model-autonomous gates --spec {resp['spec_id']}")
    return 0
...
def _status_one(spec_id: str) -> int:
    ...
    if open_gates:
        ...
        for g in open_gates:
            ...
            print(f"    coding-model-autonomous review {g['id']} --approve")
...
def cmd_gates(args: argparse.Namespace) -> int:
    ...
    for g in gates:
        ...
        print(f"  coding-model-autonomous review {g['id']} --approve")
        print(f"  coding-model-autonomous review {g['id']} --reject --notes 'why'")
        print()
    return 0
