...
def migrate_legacy_sessions():
    """One-time migration from flat files in ~/ to ~/.coding_model_sessions/."""
    ensure_session_dir()
    migrated = 0
    # Default session
    legacy_default = os.path.expanduser("~/.coding_model_chat_history.json")
    ...
    # Named sessions
    for legacy in glob.glob(os.path.expanduser("~/.coding_model_chat_history_*.json")):
        name = os.path.basename(legacy).replace(".coding_model_chat_history_", "").replace(".json", "")
