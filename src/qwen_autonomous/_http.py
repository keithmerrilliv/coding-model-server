"""Shared HTTP access to the qwen-server inference API for autonomous agents.
...
Server-side counterpart to ``qwen_client/http.py`` — kept separate on purpose:
this package talks to the *local* server (``QWEN_SERVER_IP``/``QWEN_SERVER_PORT``,
...
API_URL = f"http://{QWEN_SERVER_HOST}:{QWEN_SERVER_PORT}/v1/chat/completions"
...
def post_chat_completion(model, messages, *, timeout, skip_memory=True,
    ...
    """POST a chat completion to the local qwen-server; return raw Response.
