...
| `AUTONOMOUS_MEMORY_LANGUAGES` | `swift,objective-c,objective-c++` | Comma-separated spec languages the retrieval corpus covers (DEV-657, DEV-781). A call retrieves only when BOTH its role and the spec's language opt in, so an Apple-documentation corpus is not queried for Python work. Names are normalised (`objc`, `Obj-C`, `objcpp` all count). The language is the plan's `language:` field; when the plan has none, the implement phase's file extensions decide (`.mm` → objective-c++, `.m` → objective-c, `.swift` → swift), and a spec with neither does not retrieve. |
...
### Phase b — adversarial test generation (REMOVED)

Removed in DEV-717 (2026-09-17). Over its whole life the stage ran 15
times and added **one** test: 9 runs returned nothing the parser
accepted and 5 hit a Gemini 503. It was the pipeline's only
external-API dependency in the autonomous path. The
`AUTONOMOUS_ADVERSARIAL_*` variables no longer do anything and can be
deleted from any `.env`.

The Gemini and Claude judges themselves remain — `/review` still uses
them (see below).
