...
**Runs 17 to 40 — the pipeline turned on itself.** After run 16 the target
...
way, and the kernel refactor ([CHANGELOG](CHANGELOG.md)) is their sum. Runs 31
and 39 are the exceptions that matter — both went back to the Swift target on
the Mac runner, and run 39 is the first time a suite this pipeline wrote and
ran in Swift went green end to end.
